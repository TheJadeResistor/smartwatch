...
void sendAllCodes() 
{
  ...
  if (regionNA) { //digitalRead(REGIONSWITCH)
    region = NA;
...
void appTVbGone(void){
//setup
  Serial.println("TVbGone");
  irsend.begin();
  ...
  if (regionNA){ //region == NA
    //quickflashLEDx(3);
    Serial.println("NA codes selected");
  }
  else{ //region == EU
    //quickflashLEDx(6);
    Serial.println("EU codes selected");
  }
...
//loop
  while(1){
    if(ttgo->getTouch(x, y)) {  // if a touch then leave
      if((x>70 && x<170) && (y>70 && y<140)) //send button
        sendCrap();
      else if((x>85 && x<165) && (y>190 && y<230)) //exit button
        break;
    }
    //yield(); //not needed
...
void setupTV(){
  ...
  ttgo->tft->drawCentreString("TV-b-gone",  half_width, 10, 2);

//draw centered buttons
  //ttgo->tft->fillRoundRect(70, 65, 100, 70, 5, TFT_DARKCYAN); //x, y, w, h, radius //send button
  //ttgo->tft->fillRoundRect(85, 190, 75, 40, 5, TFT_DARKCYAN); //x, y, w, h, radius //exit button
  //ttgo->tft->drawCentreString("Send",  120, 85, 2); //sting, x, y, font
  //ttgo->tft->drawCentreString("Exit",  115, 195, 2); //sting, x, y, font

//draw good buttons
  ttgo->tft->fillRoundRect(140, 105, 80, 120, 5, TFT_DARKCYAN); //x, y, w, h, radius //send button
  ttgo->tft->fillRoundRect(5, 185, 60, 50, 5, TFT_DARKCYAN); //x, y, w, h, radius //exit button
  ttgo->tft->setTextSize(2);
  ttgo->tft->setTextColor(TFT_BLACK); //, TFT_DARKCYAN);
  ttgo->tft->drawCentreString("Send",  177, 150, 2); //string, x, y, font
  ttgo->tft->drawCentreString("Exit",  35, 195, 2); //string, x, y, font
}
